 ...
 */
public abstract class LoggableBleManager_2<T extends BleManagerCallbacks> extends LegacyBleManager<T> {
//	private ILogSession logSession;
	 ...
	 */
	public LoggableBleManager_2(@NonNull final Context context) {
		super(context);
	}
...
//			Toast.makeText(getContext(),"연결 끊김",Toast.LENGTH_SHORT).show();
			BleConnectionActivity.getBleConnectionActivity().setToast("'팔1' 에 장착되어 있는 기기와의 연결이 끊어졌습니다.");
			...
			if(MainActivity.getMain() !=null) {
				MainActivity.getMain().device2Vis();
			}
